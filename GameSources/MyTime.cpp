...
namespace basecross {
	...
	void MyTime::OnCreate() {
		...
		GetStage()->SetSharedGameObject(L"Time", GetThis<MyTime>());         //!シェアドオブジェクトに読み込ませる
	}

	
	void MyTime::SetUVPos( int&verNum,UINT num, vector<VertexPositionTexture>& newVertices)
	{       
