...
namespace basecross {
	...
	void Scene::CreateResourses() {
		...
		strTexture = dataDir + L"PushButtonX.png";
		App::GetApp()->RegisterTexture(L"PushButtonX_TX", strTexture);
		...
		//モデル
		//ボーンモデルの通常リソース
		auto multiModelMesh = MeshResource::CreateBoneModelMesh(dataDir, L"player.bmf");
		App::GetApp()->RegisterResource(L"Object_WalkAnimation_MESH", multiModelMesh);

		//ボーンモデルのタンジェント付きリソース
		multiModelMesh = MeshResource::CreateBoneModelMeshWithTangent(dataDir, L"player.bmf");
		...
		//落石読み込み
		auto staticModelMesh7 = MeshResource::CreateStaticModelMesh(dataDir, L"Foring1.bmf");
